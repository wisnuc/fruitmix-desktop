  ...
  constructor(type, paths, target) {
    ...
    this.roots = []
      ...
      paths.forEach(abspath => {
        this.roots.push(createFileUploadTask(null, abspath, target))
      })
    }
    else {
      paths.forEach(abspath => {
        this.roots.push(createFolderUploadTask(null, abspath, target))
      })
    }
	}
}

const createUserTask = (type, paths, target) => {
  let userTask = new UserTask(type, paths, target)
...
const createFileUploadTask = (parent, abspath, target) => {
  let task = new fileUploadTask(parent, abspath, target)
...
class fileUploadTask extends EventEmitter {
	...
	constructor(parent, abspath, target) {
		super()
  ...
  enterRunningState() {
    ...
    this.handle = request(options, (err, res, body) => {
      if (!err && res.statusCode == 200) {
        this.setState('finished', err, null)
      }else {
        this.setState('finished', null, JSON.parse(body).uuid)
      }
    })
  }
...
// factory 
const createFolderUploadTask = (parent, abspath, target) => {
  let task = new folderUploadTask(parent, abspath, target)
