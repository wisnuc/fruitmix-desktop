...
const directory = (state = defaultDirectory,action)=> {
	switch (action.type) {
		...
		case 'SET_DIRECTORY':
			var position = action.children.map((item,index)=>{
		...
		// 	for (var i =0; i < dowload.length; i++) {
		// 		dowload[i].status = 0
		// 	}
		...
		// 	// for (var i = 0;i<newUploadArr.length;i++) {
		...
		// 	for (var i=0;i<newDownloadArr.length;i++) {
