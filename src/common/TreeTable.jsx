...
class TreeTable extends React.Component {
  ...
  constructor(props) {
    ...
    this.toggleExpansion = (item) => {
      const expansion = new Set(this.state.expansion)

      if (this.state.expansion.has(item)) { expansion.delete(item) } else { expansion.add(item) }
    ...
    this.renderData = () => {
      ...
      const renderItem = (item, level) => {
        ...
        if (this.state.expansion.has(item) && item.children && item.children.length) { item.children.forEach(child => renderItem(child, level + 1)) }
      }
